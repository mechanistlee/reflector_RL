...
class SAC(OffPolicyAlgorithm):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            # Use standard discount factor (gamma) for compatibility with older SB3 versions
    ...
    def learn(
        ...
        **kwargs,
        ...
        :param eval_callback: Callback used for evaluation
        :param kwargs: Additional arguments (for compatibility)
        :return: the trained model
        """
        # Handle evaluation callback if eval_env is provided
        if eval_env is not None and eval_freq > 0:
            # Import here to avoid circular import
            from stable_baselines3.common.callbacks import EvalCallback, CallbackList
            
            # Create evaluation callback if not provided
            ...
            # Combine callbacks properly
            if callback is None:
                callback = eval_callback
            else:
                # Always create a new CallbackList with all callbacks
                if isinstance(callback, CallbackList):
                    # Extract callbacks from existing CallbackList
                    all_callbacks = list(callback.callbacks)
                    all_callbacks.append(eval_callback)
                    callback = CallbackList(all_callbacks)
                elif isinstance(callback, list):
                    # If callback is a list, add eval_callback to it
                    all_callbacks = list(callback)
                    all_callbacks.append(eval_callback)
                    callback = CallbackList(all_callbacks)
                else:
                    # If callback is a single callback, create a new CallbackList
